...
MATERIAL_DIR = os.path.join(STATIC_DIR, "materials") # 放材質遮罩與線圖
...
DEFAULT_SHOP_DATA = {
    "brands": ["蘋果", "三星"],
    "models": [
        {"id": "model_apple_ip17pm", "brand": "蘋果", "name": "iPhone 17 Pro Max", "status": True}
    ],
    "styles": [
        {
            "id": "style_clear_01", 
...
def admin_upload_image():
    ...
    try:
        ...
        upload_type = request.form.get('type', 'sticker') # sticker 或 material
        ...
        filename = f"{prefix}{int(time.time()*1000)}_{file.filename}"
        filepath = os.path.join(target_dir, filename)
        file.save(filepath)
        
        folder_name = "materials" if upload_type == 'material' else "stickers"
        url = f"/static/{folder_name}/{filename}"
    ...
    try:
        ...
        for file in files:
            if file.filename:
                ...
                file.save(filepath)
                assets['stickers'].append({"id": filename, "category": category, "url": f"/static/stickers/{filename}"})
                count += 1
