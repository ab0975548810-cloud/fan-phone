(function(){
  ...
  function card(row){
    ...
    if(j?.state==='QUEUED')actions+=`<button class="primary" data-pc="start" data-job="${esc(j.id)}" ${window.pcVendorReady?'':'disabled title="雲打印安全設定未完整"'}>開始列印</button><button class="danger" data-pc="cancel" data-job="${esc(j.id)}" ${window.pcVendorConnected?'':'disabled'}>取消等待</button>`;
