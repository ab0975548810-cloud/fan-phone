"""Print Phase 3.0 domain, safe vendor handoff, callbacks, and admin API."""
...
STATE_LABELS = {
    "PREPARED": "待送出", "SENDING": "送出中", "QUEUED": "雲端等待",
    "STARTING": "啟動中", "PRINTING": "列印中", "CANCELING": "取消中",
...
class PrintService:
    ...
    def save_profile(self, payload):
        ...
        if not channel or len(channel) > 100:
            raise PrintError("BAD_PROFILE", "請設定廠商 channel", 400)
        profile = {
            "sku_id": sku_id,
            ...
            "copies": copies,
            "spot_color": str(payload.get("spot_color") or "")[:100],
            "channel": channel,
    ...
    def start(self, job_id, key):
        fingerprint = _payload_hash("start", job_id)
        replay = self._existing_operation(key, "start", job_id, fingerprint)
        if replay:
            return replay
        job = self.store.job(job_id)
        if not job:
        ...
        order = self._order(job["order_id"])
        self._assert_order_printable(order)
        if not job.get("profile_complete") or not job.get("vendor_taskid"):
            raise PrintError("PRINT_NOT_READY", "列印參數或雲端 taskid 尚未就緒")
        if job["state"] == "UNKNOWN":
        ...
        if not owner:
            return self.store.job(job_id)
        claimed = self.store.patch_job(job_id, {"state": "STARTING", "last_error": None, "ambiguous_operation": None}, ("QUEUED",))
        if not claimed:
            self.store.finish_request(key, "FAILED", {"code": "CONCURRENT_OPERATION"})
            raise PrintError("CONCURRENT_OPERATION", "另一個列印操作正在執行")
        try:
            ...
            self.store.patch_job(job_id, {"state": "QUEUED", "last_error": "啟動前訂單已作廢"}, ("STARTING",))
            self.store.finish_request(key, "FAILED", {"job_id": job_id, "code": "VOID_ORDER"})
            raise
        try:
            result = self.client.start_print(claimed["vendor_taskid"])
        except VendorAmbiguous as exc:
            job = self.store.patch_job(job_id, {"state": "UNKNOWN", "ambiguous_operation": "startPrint", "last_error": str(exc)[:500]}, ("STARTING",))
            self.store.finish_request(key, "UNKNOWN", {"job_id": job_id, "code": "RECONCILE_REQUIRED"})
            raise PrintError("RECONCILE_REQUIRED", "啟動結果不明，禁止再次啟動；請先查核", 503)
        except (VendorDisabled, VendorError) as exc:
    ...
    def cancel(self, job_id, key):
        ...
        if job["state"] == "PREPARED":
            ...
            return updated
        if job["state"] != "QUEUED":
            raise PrintError("BAD_PRINT_STATE", "只能取消尚未開始列印的任務")
        if not self.client.ready:
            raise PrintError("VENDOR_DISABLED", "雲打印未啟用或設備憑證未設定", 503)
        owner, _ = self.store.claim_request(key, "cancel", job_id, fingerprint)
        if not owner:
...
def install(app_module):
    ...
    def inject_print_center(response):
        if request.path == "/admin" and response.status_code == 200 and response.mimetype == "text/html":
            ...
            src = "/static/admin-print-center.js?v=20260918a"
            if src not in html:
                response.set_data(html.replace("</body>", f'<link rel="stylesheet" href="/static/admin-print-center.css?v=20260918a"><script src="{src}"></script></body>'))
